using Content.Client._Sunrise.Shaders.Bloom;
using Content.Shared._Sunrise.SunriseCCVars; // FIsh edit - настройки bloom в основной вкладке графики
...
public sealed partial class GraphicsTab : Control
{
    ...
    public GraphicsTab()
    {
        ...
        // FIsh edit start - пользовательские настройки bloom в основной вкладке графики
        Control.AddOptionCheckBox(SunriseCCVars.LightBloomEnabled, LightBloomEnabledCheckBox);
        var bloomStrength = Control.AddOptionPercentSlider(
            SunriseCCVars.LightBloomStrength,
            LightBloomStrengthSlider);
        LightBloomStrengthSlider.Slider.Rounded = true;
        LightBloomStrengthSlider.Slider.RoundingDecimals = 2;
        bloomStrength.ImmediateValueChanged += PreviewBloomStrength;
        LightBloomEnabledCheckBox.OnToggled += _ => UpdateLightBloomOptionsVisibility();
        Control.ValuesReset += PreviewCurrentBloomStrength;
        // FIsh edit end
        ...
        UpdateLightBloomOptionsVisibility(); // FIsh edit
    }

    // FIsh edit start - скрываем интенсивность при отключённом bloom
    private void UpdateLightBloomOptionsVisibility()
    {
        LightBloomStrengthSlider.Visible = LightBloomEnabledCheckBox.Pressed;
    }

    private void PreviewBloomStrength(float strength)
    {
        _entityManager.System<LightingOverlaySystem>().PreviewBloomStrength(strength);
    }

    private void PreviewCurrentBloomStrength()
    {
        PreviewBloomStrength(LightBloomStrengthSlider.Slider.Value);
    }
