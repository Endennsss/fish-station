...
/// <summary>Клиентский предпросмотр dissolve для выбранного локального EntityUid.</summary>
public sealed class DissolveCommand : LocalizedEntityCommands
{
    [Dependency] private readonly DissolveSystem _dissolve = default!;

    public override string Command => "fish_dissolve";
    ...
    public override void Execute(IConsoleShell shell, string argStr, string[] args)
    {
        ...
        if (args.Length > 5)
        {
            if (Color.TryFromHex(args[5]) is not { } color)
            {
                shell.WriteError(Help);
                return;
            }
        ...
        if (!success)
            shell.WriteError(Loc.GetString("fish-dissolve-failed"));
    }
